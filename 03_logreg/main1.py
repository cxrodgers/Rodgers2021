...
# main1 is for running individual models
...
for stratify_by in stratify_by_l:
    ...
    results_dir = os.path.join(params['logreg_dir'], 'indiv_model_results')
    ...
    for dataset in datasets:
        ...
        for component in features.columns.levels[0]:
            print(component)

            # Extract component, maintaining the levels for consistency
            feature_set = features.loc[:, [component]].copy()
            
            # Store here
            dataset_component_model_dir = os.path.join(dataset_model_dir, component)
